    ...
    autoTable(doc, {
      head: [
        [
          ...
          "Total Streams",
          "Total Followers",
          "Total Private Call Duration",
          "Avg Stream Time",
          "Avg Viewers per stream",
          "Avg Diamonds per stream",
        ],
      ],
      body: data.map((row) => [
        row.id,
        ...
        row.totalStreams,
        row.totalFollowers,
        row.totalPrivateCallDuration,
        row.avgStreamTime,
        row.avgViewersPerStream,
        row.avgDiamondsPerStream,
      ]),
    });
  ...
  return (
    <div className="flex w-full items-center justify-between px-5 mt-5">
      ...
      <div className="flex w-[50%] justify-end">
        <Button
          onClick={() => setExportButton((p) => !p)}
          className="bg-zinc-600 hover:bg-zinc-700 cursor-pointer min-w-fit w-28 h-10"
        >
          Export Report
          <ChevronDown />
        </Button>
        {exportButton && (
          <div className="absolute border-0 z-20 bg-zinc-800 w-[16%] py-4 h-36 mt-10 rounded-[12px] flex flex-col justify-between">
            <p className="opacity-50 text-md ml-4">Download report as</p>
            <div
              onClick={handleCSV}
              className="text-lg cursor-pointer mx-2 px-2 py-1 rounded-lg hover:bg-zinc-600"
            >
              CSV
            </div>
            <div
