      ...
      <Table className="bg-zinc-900 flex flex-col p-4 border-0 rounded-xl">
        <TableHeader className='w-full'>
          <TableRow className="hover:bg-transparent border-white/50 flex justify-between w-full items-center">
            ...
            <TableHead style={{ padding: 0 }} className='text-white flex gap-1.5 w-[20%]'><ChevronsUpDown onClick={() =>{setAmountClicked((p) => !p); sortTransactions(items, { field: 'payoutAmount', ascending: amountClicked })}} className='cursor-pointer' size={21} />Payout amount</TableHead>
            <TableHead style={{ padding: 0 }} className='text-white flex gap-1.5 w-[20%]'><ChevronsUpDown onClick={() => {setDateClicked((p) => !p); sortTransactions(data, { field: 'date', ascending: dateClicked })}} className='cursor-pointer' size={21} />Date</TableHead>
            <TableHead style={{ padding: 0 }} className="text-white flex gap-1.5 w-[20%]"><ChevronsUpDown onClick={() => {setStatusClicked(true)}} className='cursor-pointer' size={21} />Status
              {statusClicked &&
