...
const FeedbackForm = ({ setMenu }: { setMenu: React.Dispatch<SetStateAction<number>> }) => {
  ...
  return (
    <div className="p-6 bg-zinc-800 rounded-lg w-[40vw] text-gray-100">
      <div className='flex justify-between'>
        ...
        <CrossIcon onClick={()=>setMenu(-1)} className='rotate-45 cursor-pointer'/>
      </div>
      {showSuccessNotification && (
                <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
                  <div className="bg-[#1c1c1c] text-white rounded-xl p-6 w-[320px] text-center relative shadow-lg">
                    <button
                      onClick={() => {setShowSuccessNotification(false); setMenu(-1)}}
